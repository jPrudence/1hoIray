...
function 1hoiray( $isa_nbr, $dilatrin_n_ny_tapitrisa = false){
	...
	elseif($isa_nbr < 100){
		if($isa_nbr % 10 == 0)
			...
			$isa_str = 1hoiray($isa_b) . ($isa_a == 0?"":($isa_a == 1?" ambin'ny ":" amby ") . 1hoiray($isa_a*10));
	}
	elseif($isa_nbr < 1000){
		if($isa_nbr % 100 == 0)
			...
			$isa_str = 1hoiray($isa_b) . (( ($isa_b % 10==0 || $isa_b < 100) && $isa_nbr <= 200 )?" amby ":" sy ") . 1hoiray($isa_a*100);
	}
	else{
		if($isa_a == 1 && $isa_nbr < 10000)
			$isa_str = ($isa_nbr != 1000?1hoiray($isa_b) . " sy ":"") . "arivo";
		else{
			...
			$isa_str = ($isa_b == 0?'':(1hoiray($isa_b) . ($isa_b<10?" amby ":" sy ") ) ) . 1hoiray( $isa_a) . " " . ($isa_a == 0?"":$isa_vaventy[$index]);
		}
	}

	return str_replace(["tapitrisa tapitrisa", "iray amb", " sy aotra"], ["lavitrisa", "iraika amb", ""], $isa_str);
}
