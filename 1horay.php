...
function isa1hoiray( $isa_nbr, $dilatrin_n_ny_tapitrisa = false){
	...
	elseif($isa_nbr < 1000){
		if($isa_nbr % 100 == 0)
			...
			$isa_str = isa1hoiray($isa_b) . (( ($isa_b % 10==0 || $isa_b < 100) && $isa_nbr <= 200 )?" amby ":" sy ") . isa1hoiray($isa_a*100);
	}
	else{
		if($isa_a == 1 && $isa_nbr < 10000)
			$isa_str = ($isa_nbr != 1000?isa1hoiray($isa_b) . " sy ":"") . "arivo";
		else{
